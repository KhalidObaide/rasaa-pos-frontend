...
const Types = () => {
  ...
  async function getData() {
    const token =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmcmVzaCI6ZmFsc2UsImlhdCI6MTY5MzY1MTU1MywianRpIjoiOTVjY2ZkYTMtZTRiMS00OTc5LWJlMGEtODQ2OGYyYjRmZDY4IiwidHlwZSI6ImFjY2VzcyIsInN1YiI6ImtoYWxpZCIsIm5iZiI6MTY5MzY1MTU1MywiZXhwIjoxNjkzNzM3OTUzfQ.Be7V5xlviiP3JkLmgvwSxL5K7qPhiSZwfE9pHoUaBz4";
    try {
      const response = await axios.get(
        "https://lajward-mis.dev:8005/utilities",
    ...
    <div className=" font-Estedad flex flex-col justify-center items-center shadow-md shadow-[rgb(235, 235, 235) rounded-md  mx-5 p-4 ">
      ...
      <div className=" w-full flex flex-row-reverse justify-between items-center mb-5">
        <button
          ...
          className=" bg-btn text-white text-2xl py-3 px-5  pb-5 font-medium rounded-md "
        >
          ثبت مورد جدید
        </button>
      ...
      <div className="w-full">
        <table className="w-full">
          <thead>
            <tr className=" bg-charts ">
              <td className="w-60   text-2xl text-center py-5 font-semibold ">
                No
              </td>
              <td className="w-324 text-2xl text-center font-semibold">
                عنوان
              </td>
          ...
          <tbody>
            {data == null
              ...
              : data.map((data: any, index: any) => (
                  ...
                  >
                    <td className="w-60  text-center text-2xl py-5">
                      {index + 1}
                    </td>
        ...
        <div className="w-full flex flex-row-reverse justify-center items-center py-5 mt-5">
          <button className="flex flex-row justify-center items-center border-solid border border-gray_fac gray w-16 h-11 rounded-md mx-1 text-2xl p-2">
            <BiChevronLeft />
          </button>
          <button className="flex flex-row justify-center items-center border-solid border border-gray_fac text-black w-16 h-11 rounded-md mx-1 text-2xl p-2 pb-5 ">
            {douth}
          </button>
          <button
            className={`flex flex-row justify-center items-center border-solid border border-gray_fac text-black w-16 h-11 rounded-md mx-1 text-2xl p-2 ${
              style == 3 ? "bg-B_blue  text-white" : "bg-white"
            } `}
          >
            {three}
          </button>
          <button
            className={`flex flex-row justify-center items-center border-solid border border-gray_fac text-black w-16 h-11 rounded-md mx-1 text-2xl p-2 ${
              style == 2 ? "bg-B_blue  text-white" : "bg-white"
            } `}
          >
            {tow}
          </button>
          <button
            className={`flex flex-row justify-center items-center border-solid border border-gray_fac text-black w-16 h-11 rounded-md mx-1 text-2xl p-2 bg-blue ${
              style == 1 ? "bg-B_blue  text-white" : "bg-white"
            } `}
          >
            {one}
          </button>
          <button className="flex flex-row justify-center items-center border-solid border border-gray_fac text-black w-16 h-11 rounded-md mx-1 text-2xl p-2">
            <BiChevronRight />
          </button>
        </div>
      </div>
