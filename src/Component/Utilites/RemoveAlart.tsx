...
const RemoveAll = ({
  ...
  return (
    <div
      className={` fixed  top-0 left-0 right-0 bottom-0 ${style.row} items-center justify-center w-full $hidden bg-[rgba(0,0,0,0.3)] h-[100%]`}
    >
      <div className={`w-[25%] bg-white rounded-xl`}>
        ...
        <div className={`${style.col} items-center justify-center gap-y-5 p-5`}>
          ...
          >
            ...
            {/* <Link to="/Sell"> */}
            {deletLoader == true ? (
              <button className="px-5 py-2 text-white rounded-md bg-error">
                <TbLoader className="animate-spin bg-error w-5 h-5" />
              </button>
            ) : (
              <button
                onClick={() => startDelet()}
