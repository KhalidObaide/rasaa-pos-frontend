    ...
    async function getData() {
      ...
      try {
        const response = await axios.delete(
          `http://127.0.0.1:9005/utilities?id=${clickedItem.id}`,
      ...
      <div className=" w-637 p-5 bg-white absolute z-index-4 top-1/4 right-31%   flex flex-col justify-start items-center rounded-md">
        ...
        <div className="flex flex-row-reverse justify-between item-center w-full mb-16 ">
          <div className="flex flex-row justify-center items-center ">
              ...
              {display2 && (
                <>
                  <button
                    ...
                    className="text-5xl mr-5 mt-1 border border-editGreen p-0"
                  >
                    <BiEdit className="w-10 h-10 text-editGreen " />
                  </button>
                  <button
                    onClick={() => handelDelet()}
                    className="text-5xl mr-5 mt-1 border border-delet p-0"
                  >
                    <AiOutlineDelete className=" w-10 h-10 text-delet" />
                  </button>
                </>
              )}
              {display && (
                <div className="flex flex-row justify-center items-center ">
                  <button
                    ...
                    className=" text-5xl  mt-1 border border-delet p-0"
                  >
                    <IoIosClose className="text-delet m-0  " />
                  </button>
                  <button className="text-5xl mr-5 mt-1 border border-editGreen p-0">
            ...
            <button
              onClick={() => setShowEdit(false)}
              className="mb-1 text-5xl mr-5 ml-7"
            >
              &times;
            </button>
          </div>
