  ...
  return (
    <>
      <div
        ...
        className=" w-full h-full fixed top-0 left-0 z-index-5  bg-shadow"
      ></div>
      <div className=" p-5 bg-white z-index-10 rounded-md fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2">
        ...
        <div className="flex flex-row-reverse justify-between item-center w-full mb-5 ">
          <div className="flex flex-row justify-center items-center ">
            <div className="flex flex-row justify-center items-center">
              {display2 && (
                <>
                  <button
                    onClick={() => checkToEdit()}
                    className="text-2xl mr-5 mt-1 rounded-md border border-editGreen p-0"
                  >
                    <TbEditCircle className="w-7 h-7 text-editGreen text-base " />
                  </button>
                  <button
                    onClick={() => handelDelet()}
                    className="text-2xl mr-2 mt-1 border border-delet rounded-md p-0"
                  >
                    <RiDeleteBinLine className=" w-7 h-7 text-delet" />
                  </button>
                </>
              )}
              {display && (
                <div className="flex flex-row-reverse justify-center items-center ">
                  <button
                    ...
                    className=" text-2xl mr-2  mt-1 border border-delet p-0 rounded-md"
                  >
                    <IoIosClose className="text-delet m-0 " />
                  </button>
                  <button
                    onClick={() => hnadelEdit()}
                    className="text-2xl mt-1 border border-editGreen p-0 rounded-md"
                  >
                    <BsCheck className="text-editGreen" />
                  </button>
                </div>
              )}
            </div>
            <button
              onClick={() => setShowEdit(false)}
              className="mb-1 text-3xl ml-2 text-gray_fac mr-3"
            >
              &times;
            </button>
          </div>
          ...
          <div className="flex flex-row-reverse justify-around items-start">
            ...
            <div className="flex flex-col justify-center items-start">
              ...
              <input
                value={title}
                onChange={(e) => {
                  setTitle(e.target.value), setUbdateTitle(e.target.value);
                }}
                className="w-80 h-12  py-2 px-2 pr-5 text-right border-solid border border-graybutton rounded-md outline-none mx-2 text-lg "
              />
            </div>
      ...
      <div className="flex flex-row-reverse justify-around items-start"></div>
      {dispalyDelet && (
        <RemoveAll
          startDelet={startDelet}
          deletLoader={deletLoader}
          setDispalyDelet={setDispalyDelet}
          dispalyDelet={dispalyDelet}
        />
      )}
    </>
  );
